...
export const Button: React.FC<ButtonProps> = ({children, onClick=() => {}}): JSX.Element => (
  <button type="button" className="btn btn-reef" onClick={onClick}>
    {children}
  </button>
)

export const Empty: React.FC<ButtonProps> = ({children, onClick=() => {}}): JSX.Element => (
  <button type="button" className="btn" onClick={onClick}>
    {children}
  </button>
)

export const Nav: React.FC<NavProps> = ({ to, children, selected = false }): JSX.Element => (
  <Link to={to} className={`border-rad h-100 fs-6 fw-bold px-3 py-2 ${selected ? 'nav-selected' : 'nav-button'}`}>
    {children}
  </Link>
);
...
export const SwitchToken = ({ addIcon, disabled, onClick }: SwitchTokenButton): JSX.Element => (
...
export const Icon: React.FC<IconButton> = ({ onClick, children }): JSX.Element => (
