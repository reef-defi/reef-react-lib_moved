...
const fetchTokensData = (
  ...
  apollo
    ...
    // eslint-disable-next-line camelcase
    .then((verContracts) =>
      verContracts.data.verified_contract.map(
...
// eslint-disable-next-line camelcase
const tokenBalancesWithContractDataCache =
  (apollo: ApolloClient<any>) =>
  (
    state: { tokens: Token[]; contractData: Token[] },
    ...
    const missingCacheContractDataAddresses = tokenBalances
      .filter(
        (tb) =>
          !state.contractData.some((cd) => cd.address === tb.token_address)
      )
...
export const selectedSignerTokenBalances$: Observable<Token[]> = combineLatest([
  ...
  switchMap(([apollo, signer, provider]) =>
    !signer
      ? []
      : zenToRx(
          apollo.subscribe({
          ...
          map((res: any) =>
            res.data && res.data.token_holder
          ...
          switchMap(
            async (
              ...
              reefTokenResult.balance =
                FixedNumber.fromValue(reefBalance).toUnsafeFloat();
              return Promise.resolve(tokenBalances);
            }
          ),
          // eslint-disable-next-line camelcase
...
export const pools$: Observable<Pool[]> = combineLatest([
  ...
  switchMap(([tkns, network, signer]) =>
    signer ? loadPools(tkns, signer.signer, network.factoryAddress) : []
  ),
  shareReplay(1)
);
    ...
    switchMap(([apollo, signer]) =>
      !signer
        ...
        : zenToRx(
            ...
            map((res: any) =>
              res.data && res.data.evm_event ? res.data.evm_event : undefined
            )
          )
    ),
    filter((v) => !!v),
