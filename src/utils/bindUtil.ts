...
export const bindEvmAddress = (signer: ReefSigner, provider: Provider, onTxChange?: TxStatusHandler, generateDefault?: boolean): string => {
  ...
  if (signer && displayedPopup.indexOf(signer.address) < 0 && !signer?.isEvmClaimed) {
    // eslint-disable-next-line no-restricted-globals,no-alert
    const isDefault = generateDefault || confirm('Enable Reef chain with Ethereum VM capabilities.');
