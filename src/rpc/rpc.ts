...
export const getREEF20Contract = async (
  address: string,
  signer: Signer,
): Promise< {contract: Contract, values: {name: string, symbol: string, decimals: number} }|null> => {
  try {
...
export const contractToToken = async (tokenContract: Contract, signer: ReefSigner): Promise<Token> => {
