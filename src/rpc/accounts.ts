...
export const getExtensionSigners = async (
  ...
  const extensionAccountPromises = extensions.map((ext) =>
    ext.accounts
      .get()
      .then((extAccounts) =>
        accountsToSigners(
  ...
  return Promise.all(extensionAccountPromises).then((signersByExt) =>
    signersByExt.reduce((all, curr) => all.concat(curr), [])
  );
};
